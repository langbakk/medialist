...
require_once('conf/config.php');
	...
	if (file_exists($userpath.$username.$symbolic)) {
		if (!empty($symbolic)) {
			...
			$new_symbolic = ($checkthumbs[0] == 'video') ? $checkthumbs[0].'/'.explode('/',$username)[0].'__'.$checkthumbs[1].'.jpg' : $checkthumbs[0].'/'.explode('/',$username)[0].'__'.$checkthumbs[1];
			if (!is_link($userpath.'public/'.$checkthumbs[0].'/thumbs/'.explode('/',$username)[0].'__'.$checkthumbs[1])) {
				$thumbs = ($checkthumbs[0] == 'pictures' || $checkthumbs[0] == 'video') ? symlink($_SERVER['DOCUMENT_ROOT'].'/'.$userpath.$username.$checkthumbs[0].'/thumbs/'.$checkthumbs[1],$userpath.'public/'.$checkthumbs[0].'/thumbs/'.explode('/',$username)[0].'__'.$checkthumbs[1]) : false;
			}
